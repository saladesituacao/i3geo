...
var fluxo = {
	...
	afterGetData: function({node,tipoorigem,idgrupo,nomegrupo,data,query,nomesProcedimentos}){
	    ...
	    var idlayer,layer;
	    //cria os layers para cada origem e destino
	    jQuery.each(data.chaves, function( index, value ) {
		var linhas = fluxo.makeLines(data.dados[value],data.totalocorrencias*1);
		var idlayer = i3GEO.util.uid()+"a"+index;
		layer = fluxo.createLayer(data.dados[value][0].nomecompleto,data.dados[value][0].nome_destino,idlayer,fluxo._coresCnes[value],(value != 'DF' ? true:false));
		layer.set("nomegrupo",nomegrupo);
		layer.set("fluxos",data.dados[value]);
		layer.set("tipoorigem",tipoorigem);
		layer.set("cnes_destino",value);
		layer.set("nomesProcedimentos",nomesProcedimentos);
		fluxo._grupos[idgrupo].idslayers.push(idlayer);
		fluxo.layerAddLines(layer,linhas);
	    });
	    //cria um layer com o total de ocorrencias na origem
	    idlayer = i3GEO.util.uid()+"b";
	    layer = fluxo.createLayer("","Total por origem",idlayer,"255,255,255",false);
	    layer.set("tipoorigem",tipoorigem);
	    layer.set("nomegrupo",nomegrupo);
	    layer.set("fluxos",data.dados.DF);
	    layer.set("cnes_destino","nenhum");
	    fluxo._coresCnes.nenhum = "255,0,0";
	    layer.set("nomesProcedimentos",nomesProcedimentos);
	    layer.set("numocorrencias",data.totalocorrencias*1);
	    fluxo.layerAddCirles(layer);
	    fluxo._grupos[idgrupo].idslayers.push(idlayer);
	...
	addOptionsGroup : function(id,nomesProcedimentos,node){
		...
		if(fluxo._coresCnes[layer.get("cnes_destino")]){
		    var hash = {
			    ...
			    "idgrupo": id,
			    "idlayer": idlayer,
			    "numocorrencias": layer.get("numocorrencias"),
	...
	makeLines : function(data,totalocorrencias){
	    fluxo.log("makeLines",(new Error).lineNumber);
	    var linhas = [];
	    var apenasride = $i("apenasride").checked;
	    var tipoorigem = $i("tipoorigem").value;
	    var fatorwidth = 1, i,n,d;
	    n = data.length;
	    fatorwidth = 10 / totalocorrencias;
		...
		if(apenasride && tipoorigem == "uf"){
		    d.x1 = centroideUfride[d.nome_origem].x;
		    d.y1 = centroideUfride[d.nome_origem].y;
		}
		linhas.push({
		    "linestring":[[d.x2*1,d.y2*1],[d.x1*1,d.y1*1]],
		    "origem":d.nome_origem,
		    "tipo_origem":d.tipo_origem,
		    "destino": d.nome_destino,
		    "numocorrencias": d.numocorrencias,
		    "codigo_destino": d.codigo_destino,
		    "w": 1 + (d.numocorrencias * fatorwidth)
		});
	    }
	    return linhas;
	},
	layerAddLines: function(layer,linhas){
	    fluxo.log("layerAddLines -> ",(new Error).lineNumber);
	    var n = linhas.length, i, linha, ocorrencias = 0, nw = 0;
	    for(i = 0; i < n; i++){
		var l = fluxo.layerAddLine(layer,linhas[i]);
		nw = nw + l[0];
		ocorrencias = ocorrencias + linhas[i].numocorrencias*1
	    }
	    layer.set("numocorrencias",ocorrencias);
	    //adiciona o circulo com o total de ocorrencias
	    var linha = linhas[0];
	    var geometria = i3GEO.util.projGeo2OSM(
		    new ol.geom.Circle(linha.linestring[0],0.0001)
	    );
		...
		numocorrencias: ocorrencias,
		circle: true
	    });
	    feature.setId(linha.id+"pointTotal");
	    feature.setStyle(
		    new ol.style.Style({
	...
	layerAddLine:function(layer,linha){
	    fluxo.log("layerAddLine",(new Error).lineNumber);
	    var geometria = i3GEO.util.projGeo2OSM(
		    new ol.geom.LineString(linha.linestring)
	    );
	    var wi = linha.w * fluxo._strokeWidth;
	    var feature = new ol.Feature({
		geometry: geometria,
	...
	clickFeature: function(e){
		...
		if( feature.getProperties() && feature.getProperties().nome_origem){
		    i3GEO.janela.tempoMsg(fluxo._tituloOrigem[feature.getProperties().tipo_origem] + " - " + feature.getProperties().nome_origem + " -> " + feature.getProperties().nome_destino + "<br>" + feature.getProperties().numocorrencias);
		}
	    },{hitTolerance : 5});
	},
	layerAddCirles: function(layer){
	    fluxo.log("layerAddCirles",(new Error).lineNumber);
	    var data = layer.get("fluxos");
	    var totalocorrencias = layer.get("numocorrencias")*1;
	    var features = [];
	    $.each(data,function(i,v){
		var geometria = i3GEO.util.projGeo2OSM(
			new ol.geom.Circle([v.x1*1,v.y1*1],0.0001)
		);
		var feature = new ol.Feature({
		    geometry: geometria,
		...
		feature.setStyle(
			new ol.style.Style({
				...
				width: (v.numocorrencias * 100 / totalocorrencias) + 12
			    }),
			    fill: new ol.style.Fill({
				color: 'rgba(255,0,0,0.5)'
			    })
	...
	show: {
	    ...
	    circlesonoff: function(idgrupo){
		var layers = fluxo._grupos[idgrupo].idslayers;
		$.each(layers, function(i,v){
		    var layer = fluxo._layersByName[v];
		    var cor = $("#" + idgrupo + v + "Color").val();
		    layer.getSource().getFeaturesCollection().forEach(function(feature){
			if(feature.getProperties().circle == true){
			    if(feature.getProperties().estilo){
				if(feature.getProperties().visivel === true){
				    feature.set("visivel",false);
				    feature.setStyle(new ol.style.Style({}));
				} else {
				    feature.set("visivel",true);
				    feature.setStyle(feature.getProperties().estilo);
				}
			    } else {
				feature.set("visivel",false);
				feature.set("estilo",feature.getStyle());
				feature.setStyle(new ol.style.Style({}));
			    }
			}
		    });
		...
		$.each(dados,function(i,v){
		    dados[i]["perc"] = (v.numocorrencias * 100) / total;
		});
